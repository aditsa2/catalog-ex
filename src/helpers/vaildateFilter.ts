...
function isArrayOfObjectsValid(arr: FilterObject[]): boolean {
  ...
  for (const filterObj of arr) {
    ...
    Object.keys(filterObj).forEach((key: string): void => {
      objectKeysValid = objectKeysValid && ['field', 'operator', 'value'].includes(key);
    });
...
const filters: FilterObject[] = [
  ...
  { field: 'description', operator: '=', value: 'Some description' },
  { field: 'bounding_polygon', operator: 'st_contains', value: 'some polygon' },
];
